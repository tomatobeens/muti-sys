  ...
  methods: {
    addNewJobIntoInputWell: function () {
      let newJob = {
        ...
        size: Number(this.newSize), // 内存需要
        need: Number(this.newNeed) // 磁带机需要
      }
    ...
    startProcess () {
      let _this = this
      let runTime = setInterval(startProcessing, 1000)
      function startProcessing () {
        // 如果作业队列和就绪队列都空就清除定时器
        if (_this.inputWell[0] === undefined && _this.waiting[0] === undefined) {
    ...
    addFirstJobToWaiting () {
      ...
      if (_this.inputWell[0] !== undefined) {
        ...
        // waiting改变就重排
        _this.waiting.sort(_this.minFirst('runTimes'))
      } else {
        console.log('作业队列已经空')
      }
    },
